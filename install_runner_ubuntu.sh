...
AUTH_TOKEN="#PROTECTED#"                                           # Auth token for CircleCI
RUNNER_NAME="circleci"                                          # A runner name - this is not the same as the Resource class - keep it short, and only with letters/numbers/dashes/underscores
UNIQUE_RUNNER_NAME="$RUNNER_NAME-$TIMESTAMP"            # Runners must have a unique name, so we'll append a timestamp
USERNAME="root"                                     # The user which the runner will execute as
...
apt update
apt install unzip coreutils curl tar gzip -y
